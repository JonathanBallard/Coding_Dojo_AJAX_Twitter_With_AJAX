...
class TweetsSchema(ma.ModelSchema):
    class Meta:
        model = Tweets
...
def tweet_create():
    ...
    return render_template('partials/tweets.html', tweetList = tweetListFinal)
    # return redirect('/dashboard')
...
def get_users():
    ...
    followedUsers = thisUser.user_being_followed
    print('$$$$$$$$$$555555', thisUser.user_following)


    return render_template('/users.html',  users = users, followedUsers = followedUsers)
...
def unfollow_user(id):
    ...
    user_to_follow = Users.query.get(id)
    user = Users.query.get(session['id'])
    user.user_being_followed.remove(user_to_follow)
...
def emailCheck():
    ...
    result = Users.query.filter_by(email = em)
...
def search_user():
    email = request.form['emailSearch']
    search = "%{}%".format(email)
    users = Users.query.filter(Users.email.like(search)).all()
    print(users, '***********************************************************')

    return render_template('partials/search.html', users = users)
